/* ═══════════════════════════════════════════════════════════
   DICT IMS v2  –  script.js
   Full frontend logic for Admin, Supervisor, and Intern roles
═══════════════════════════════════════════════════════════ */

// Resolve API base path dynamically — works in any subfolder like /ims-dict/
const API = (() => {
...
let allInterns = [];      // admin cache
let todayRec = {};        // intern today's attendance
let sessionsCache = [];
let docsCache = [];
let reportsCache = [];
...
function startClock() {
  const tick = () => {
    ...
    setEl('clock-date', n.toLocaleDateString('en-PH',{weekday:'long',year:'numeric',month:'long',day:'numeric'}));
  };
...
function initDateInputs() {
  ...
  if (m) { const n = new Date(); m.value = `${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,'0')}`; }
}
...
async function handleLogin(e) {
  ...
  try {
    ...
    // Only fall to demo if the server is completely unreachable (network error)
    if (err.data) {
      // Server responded with an error JSON
      setAuthError(err.data.error || 'Login failed.');
    } else {
      // True network/connection failure — use demo mode
...
function setAuthError(msg) { const el=document.getElementById('auth-message'); el.className='auth-message error'; el.textContent=msg; }
function togglePw() {
  const inp=document.getElementById('login-password'), icon=document.getElementById('pw-toggle');
...
function enterApp(user) {
  CU = user;
  hide('auth-modal'); show('sidebar'); show('main-content');
  ...
  document.querySelectorAll('.intern-nav').forEach(el => el.style.display = user.role==='intern' ? 'flex' : 'none');
  document.querySelectorAll('.admin-nav').forEach(el  => el.style.display = (user.role==='admin'||user.role==='supervisor') ? 'flex' : 'none');

  // Show correct dashboard
  show(user.role==='intern' ? 'intern-dash' : 'admin-dash');
  hide(user.role==='intern' ? 'admin-dash' : 'intern-dash');
  setEl('welcome-sub', user.role==='intern' ? 'DICT Isabela – OJT Intern Dashboard' : 'DICT Isabela – Admin / Management Dashboard');
  ...
  if (user.role === 'intern') {
    ...
    loadProfile();
    loadTemplates();
    loadNotifications();   // also loads intern-notif-list
  } else {
    loadAdminAtt();
    loadAdminDocs();
    loadAdminInterns();
...
async function logout() {
  ...
  CU=null;
  hide('sidebar'); hide('main-content'); show('auth-modal');
...
async function loadMyAttendance() {
  ...
  let params = `filter=${filter}&status=${status}`;
...
function renderAttTable(rows) {
  ...
  if (!el) return;
  if (!rows.length) { el.innerHTML='<div class="empty"><i class="fas fa-calendar"></i><p>No attendance records for this period.</p></div>'; return; }
  const badgeMap = {'Full Day':'badge-full','Half Day':'badge-half','Early Out':'badge-early','Absent':'badge-absent','In Progress':'badge-progress'};
  el.innerHTML = `<table>
...
async function doTime(session, action) {
  try {
    const r = await api('attendance.php', {method:'POST',body:JSON.stringify({action,session})});
    if (r.success) {
      toast(`${session} ${action==='time_in'?'Time In':'Time Out'}: ${r.time}`, 'success');
      todayRec = r.record || todayRec;
      todayRec[r.column] = r.time;
      syncTimeBtns(); updateDashAtt();
      loadMyAttendance();
...
function syncTimeBtns() {
  const dis  = id => document.getElementById(id)?.classList.add('disabled');
  const enab = id => document.getElementById(id)?.classList.remove('disabled');
  const r = todayRec;
  ...
  if (amSt) amSt.textContent = r.am_time_in ? (r.am_time_out ? `✅ In: ${r.am_time_in} | Out: ${r.am_time_out}` : `🕐 In: ${r.am_time_in}`) : 'Not Started';
  if (pmSt) pmSt.textContent = r.pm_time_in ? (r.pm_time_out ? `✅ In: ${r.pm_time_in} | Out: ${r.pm_time_out}` : `🕐 In: ${r.pm_time_in}`) : 'Not Started';
}
...
function clearAdminAttFilters() {
  ['adm-att-name','adm-att-date','adm-att-from','adm-att-to','adm-att-status'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
  loadAdminAtt();
}
  ...
  try {
    ...
    if ((r.assignments||[]).length) {
      ...
      document.getElementById('doc-assignments-list').innerHTML =
        r.assignments.map(a=>`<div style="padding:8px 0;border-bottom:1px solid var(--border);font-size:.83rem;">
          <strong>${a.title}</strong>${a.description?` — ${a.description}`:''}</div>`).join('');
    }
  } catch {}
}

function renderDocTable(docs) {
  const el = document.getElementById('doc-table'); if (!el) return;
  if (!docs.length) { el.innerHTML='<div class="empty"><i class="fas fa-file"></i><p>No documents submitted yet.</p></div>'; return; }
  const badge = s => ({pending:'badge-pending',approved:'badge-approved',rejected:'badge-rejected'}[s]||'badge-pending');
  el.innerHTML = `<table><thead><tr><th>Title</th><th>Type</th><th>Notes</th><th>Submitted</th><th>Status</th><th></th></tr></thead>
    <tbody>${docs.map(d=>`<tr>
...
function updateDocProgress(n) {
  const el = document.getElementById('doc-progress-badge'); if (!el) return;
...
function onFileSelect(inp) { const el=document.getElementById('doc-file-name'); if(el&&inp.files[0]) el.textContent='📎 '+inp.files[0].name; }
...
async function submitDocument() {
  ...
  try {
    // Use FormData so PHP can read $_POST and $_FILES
    ...
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-upload"></i> Submit Document'; }
  }
}

async function deleteDoc(id) {
  if (!confirm('Delete this document?')) return;
  try { await api(`documents.php?id=${id}`,{method:'DELETE'}); loadMyDocs(); toast('Deleted.','success'); }
  catch { docsCache=docsCache.filter(d=>d.id!==id); renderDocTable(docsCache); updateDocProgress(docsCache.length); toast('Deleted. (Demo)','success'); }
}
...
async function loadMySessions() {
  try {
    const r = await api(`sessions.php?province=${CU.province||''}`);
  ...
  const pending   = false; // track locally
  const banner    = document.getElementById('access-request-banner');
  const createBar = document.getElementById('create-session-bar');
  if (hasAccess) { if(banner)banner.style.display='none'; if(createBar)createBar.style.display='block'; }
  else           { if(banner)banner.style.display='flex'; if(createBar)createBar.style.display='none'; }
}

function renderSessionList(sessions, containerId, filter='all') {
  ...
  if (!list.length) { el.innerHTML='<div class="empty"><i class="fas fa-video"></i><p>No sessions found.</p></div>'; return; }

  el.innerHTML = list.map(s=>{
    const provinces = s.target_provinces ? s.target_provinces.split(',').filter(Boolean) : [];
    const isPast = s.session_date < now;
    return `<div class="session-item" style="${isPast?'opacity:.7':''}">
      <div class="session-item-info" style="flex:1;">
        <h4>${s.title}</h4>
        <p style="margin-top:5px;">
          <span style="margin-right:14px;"><i class="fas fa-user" style="color:var(--navy-lt);"></i> <strong>Hosted by:</strong> ${s.host_name}</span>
          <span style="margin-right:14px;"><i class="fas fa-calendar" style="color:var(--navy-lt);"></i> <strong>Date:</strong> ${fmtDate(s.session_date)}</span>
          <span style="margin-right:14px;"><i class="fas fa-clock" style="color:var(--navy-lt);"></i> <strong>Time:</strong> ${s.start_time}${s.end_time?' – '+s.end_time:''}</span>
          <span><i class="fas fa-desktop" style="color:var(--navy-lt);"></i> ${s.platform}</span>
        </p>
        ${provinces.length ? `<div class="province-pills" style="margin-top:8px;">${provinces.map(p=>`<span class="province-pill">${p}</span>`).join('')}</div>` : ''}
        ${s.meeting_link ? `<p style="margin-top:6px;font-size:.78rem;color:var(--teal);"><i class="fas fa-link"></i> <a href="${s.meeting_link}" target="_blank" style="color:var(--teal);">${s.meeting_link}</a></p>` : ''}
      </div>
      <div style="display:flex;flex-direction:column;gap:8px;align-items:flex-end;">
        ${s.meeting_link&&!isPast?`<a href="${s.meeting_link}" target="_blank" class="join-btn"><i class="fas fa-video"></i> Join</a>`:''}
        ${isPast?`<span class="badge badge-inactive">Past</span>`:`<span class="badge badge-approved">Upcoming</span>`}
      </div>
    </div>`;
  }).join('');
}
...
function filterSessions(type, btn) {
  ...
  renderSessionList(sessionsCache,'sessions-list',type);
}
...
async function requestSessionAccess() {
  ...
  try {
    ...
    r.success ? toast('Access request sent to admin!','success') : toast(r.error,'error');
  } catch { toast('Request sent! (Demo)','success'); }
...
async function loadTemplates() {
  try {
    ...
    if (tmpl.length && card && list) {
      ...
      list.innerHTML = tmpl.map(t=>`<div style="padding:8px 0;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;">
        <div><strong style="font-size:.85rem;">${t.title}</strong><p style="font-size:.77rem;color:var(--muted);">${t.description||''}</p></div>
        <a href="${t.file_path||'#'}" class="btn btn-ghost btn-sm"><i class="fas fa-download"></i> Download</a>
      </div>`).join('');
    }
  } catch {}
}
...
function renderReportTable(reports) {
  const el = document.getElementById('reports-table'); if(!el) return;
  if (!reports.length) { el.innerHTML='<div class="empty"><i class="fas fa-file-alt"></i><p>No reports submitted yet.</p></div>'; return; }
  const badge = s => ({submitted:'badge-pending',reviewed:'badge-pending',approved:'badge-approved'}[s]||'badge-pending');
  el.innerHTML = `<table><thead><tr><th>Title</th><th>Week</th><th>Date Range</th><th>Summary</th><th>Submitted</th><th>Status</th><th>Actions</th></tr></thead>
    <tbody>${reports.map(r=>`<tr>
      ...
      <td style="display:flex;gap:6px;">
        ${r.file_path&&r.file_path!=='#'?`<a href="${r.file_path}" download class="btn btn-ghost btn-sm"><i class="fas fa-download"></i></a>`:'<button class="btn btn-ghost btn-sm" disabled title="No file attached"><i class="fas fa-download"></i></button>'}
        <button class="btn btn-danger btn-sm" onclick="deleteReport(${r.id})"><i class="fas fa-trash"></i></button>
      </td>
    </tr>`).join('')}</tbody></table>`;
}

function onReportFileSelect(inp) { const el=document.getElementById('report-file-name'); if(el&&inp.files[0]) el.textContent='📎 '+inp.files[0].name; }
...
async function submitReport() {
  ...
  try {
    // Use FormData so PHP reads $_POST + $_FILES
    const fd = new FormData();
    fd.append('title',       title);
    ...
    if (r.success) {
      ...
      if (fileEl) fileEl.value = '';
      document.getElementById('report-file-name').textContent = '';
      await loadMyReports();
    } else {
      toast(r.error || 'Submission failed.', 'error');
    }
    ...
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-paper-plane"></i> Submit Report'; }
  }
}

async function deleteReport(id) {
  if (!confirm('Delete this report?')) return;
  try { await api(`reports.php?id=${id}`,{method:'DELETE'}); loadMyReports(); toast('Deleted.','success'); }
  catch { reportsCache=reportsCache.filter(r=>r.id!==id); renderReportTable(reportsCache); toast('Deleted. (Demo)','success'); }
}
...
async function saveProfile() {
  ...
  try {
    ...
    r.success ? toast('Profile saved!','success') : toast(r.error,'error');
    if (CU) CU.profile = {...(CU.profile||{}),...data};
  } catch { toast('Profile saved! (Demo)','success'); }
}
...
function renderAdminAttTable(rows) {
  const body = document.getElementById('adm-att-body'); if(!body) return;
  const badgeMap={'Full Day':'badge-full','Half Day':'badge-half','Early Out':'badge-early','Absent':'badge-absent','In Progress':'badge-progress'};
...
function renderAdminDocTable(docs) {
  const body = document.getElementById('adm-doc-body'); if(!body) return;
  const badge = s=>({pending:'badge-pending',approved:'badge-approved',rejected:'badge-rejected'}[s]||'badge-pending');
...
async function reviewDoc(id, status) {
  try {
    await api('documents.php',{method:'PUT',body:JSON.stringify({id,status})});
    toast(`Document ${status}!`,status==='approved'?'success':'error');
    loadAdminDocs();
  } catch { toast(`Marked as ${status}. (Demo)`, status==='approved'?'success':'error'); }
}
...
async function assignDocument() {
  ...
  try {
    ...
    const r = await res.json();
    if (r.success) {
      toast('Document assignment sent to intern(s)!', 'success');
      document.getElementById('adm-doc-assign-title').value = '';
      document.getElementById('adm-doc-assign-desc').value  = '';
      if (fileEl) fileEl.value = '';
      document.getElementById('adm-doc-file-name').textContent = '';
    } else {
      toast(r.error || 'Assignment failed.', 'error');
    }
  } catch (err) {
    toast('Could not connect to server.', 'error');
    console.error('assignDocument error:', err);
  }
}
...
async function loadAccessRequests() {
  try {
    ...
    const el = document.getElementById('access-requests-list'); if(!el) return;
    if (!list.length) { el.innerHTML='<div class="empty"><i class="fas fa-inbox"></i><p>No pending access requests.</p></div>'; return; }
...
async function reviewAccessReq(id, decision) {
  try {
    await api('sessions.php',{method:'PUT',body:JSON.stringify({request_id:id,decision})});
    toast(`Request ${decision}!`, decision==='approved'?'success':'error');
    loadAccessRequests(); loadAdminInterns();
  } catch { toast(`${ucfirst(decision)}. (Demo)`, decision==='approved'?'success':'error'); }
}
...
async function createSession() {
  ...
  if (!data.title||!data.session_date||!data.start_time) { toast('Fill in required fields.','error'); return; }
  try {
    const r = await api('sessions.php',{method:'POST',body:JSON.stringify(data)});
    r.success ? toast('Session created!','success') : toast(r.error,'error');
    closeModal('session-modal'); loadAdminSessions(); loadMySessions();
  } catch { toast('Session created! (Demo)','success'); closeModal('session-modal'); }
}
...
async function loadAdminReports() {
  ...
  try {
    ...
    const body = document.getElementById('adm-rpt-body'); if(!body) return;
    const badge = s=>({submitted:'badge-pending',reviewed:'badge-pending',approved:'badge-approved'}[s]||'badge-pending');
    body.innerHTML = rows.length ? rows.map(r=>`<tr>
      <td><strong>${r.full_name||'—'}</strong></td>
      <td><span class="intern-id-badge">${r.intern_id||'—'}</span></td>
      ...
      <td>${fmtDate(r.submitted_at)}</td>
      <td><span class="badge ${badge(r.status)}">${r.status}</span></td>
      <td>${r.status!=='approved'?`<button class="btn btn-success btn-sm" onclick="reviewReport(${r.id},'approved')"><i class="fas fa-check"></i> Approve</button>`:'—'}</td>
    </tr>`).join('')
...
async function reviewReport(id, status) {
  try { await api('reports.php',{method:'PUT',body:JSON.stringify({id,status})}); toast('Report approved!','success'); loadAdminReports(); }
  catch { toast('Approved. (Demo)','success'); }
}
...
async function uploadTemplate() {
  ...
  if (!fileEl?.files[0]) { toast('Please select a file to upload.', 'error'); return; }
...
async function loadAdminInterns() {
  ...
  if (body) body.innerHTML = '<tr><td colspan="9" style="text-align:center;padding:20px;color:var(--muted);"><i class="fas fa-spinner fa-spin"></i> Loading interns…</td></tr>';
  ...
  try {
    ...
    if (r.success) {
      ...
      populateInternDropdown('adm-doc-target');
    } else {
      if (body) body.innerHTML = `<tr><td colspan="9" class="empty"><i class="fas fa-exclamation-triangle" style="color:var(--red);"></i> ${r.error || 'Failed to load interns.'}</td></tr>`;
    }
  } catch (err) {
    console.error('loadAdminInterns error:', err);
    if (body) body.innerHTML = '<tr><td colspan="9" class="empty"><i class="fas fa-exclamation-triangle" style="color:var(--red);"></i> Could not load interns. Check server connection.</td></tr>';
    allInterns = [];
  }
}
...
function filterInternTable() {
  ...
  const pr = document.getElementById('im-province')?.value||'';
  const st = document.getElementById('im-status')?.value||'';
  const filtered = allInterns.filter(i=>{
    const matchQ  = !q  || (i.full_name||'').toLowerCase().includes(q)||(i.username||'').toLowerCase().includes(q)||(i.intern_id||'').toLowerCase().includes(q);
    const matchP  = !pr || i.province===pr;
...
function renderInternTable(interns) {
  const body = document.getElementById('im-table-body'); if(!body) return;
  if (!interns.length) { body.innerHTML='<tr><td colspan="9" class="empty">No interns found.</td></tr>'; return; }

  const stBadge = s => ({active:'badge-active',inactive:'badge-inactive',graduated:'badge-grad'}[s]||'badge-inactive');
  ...
  PROVINCES.forEach(province => {
    if (!grouped[province]?.length) return;
    const label = province || 'Unassigned';
    html += `<tr><td colspan="9" style="background:var(--navy-xs);padding:8px 14px;font-size:.72rem;font-weight:800;color:var(--navy);text-transform:uppercase;letter-spacing:.8px;border-bottom:2px solid var(--border);">
      <i class="fas fa-map-marker-alt" style="color:var(--gold);margin-right:6px;"></i>${label}
    </td></tr>`;
    grouped[province].forEach(i => {
      const pct = i.ojt_hours_required>0 ? Math.min(100,(i.hours_rendered/i.ojt_hours_required*100)) : 0;
      html += `<tr>
        <td><span class="intern-id-badge">${i.intern_id||'—'}</span></td>
        <td><strong>${i.full_name||'(not set)'}</strong></td>
        <td>${i.username}</td>
        <td>${i.province||'—'}</td>
        <td>
          <div style="display:flex;align-items:center;gap:8px;">
            <div class="progress-track" style="width:80px;height:10px;"><div class="progress-fill" style="width:${pct}%;"></div></div>
            <span style="font-size:.75rem;">${pct.toFixed(0)}%</span>
          </div>
        </td>
...
function updateInternStats(interns) {
  ...
  const avgOJT = interns.length ? interns.reduce((s,i)=>s+(i.ojt_hours_required>0?i.hours_rendered/i.ojt_hours_required*100:0),0)/interns.length : 0;
...
async function enrollIntern() {
  ...
  try {
    ...
    if (r.success) {
      toast('Intern enrolled successfully!', 'success');
      closeModal('enroll-modal');
      // Clear form
      ['e-id','e-username','e-password','e-hours'].forEach(id => { const el=document.getElementById(id); if(el) el.value = id==='e-hours'?'480':''; });
      document.getElementById('e-province').value = '';
      await loadAdminInterns();
    } else {
      toast(r.error || 'Enrollment failed.', 'error');
    }
  } catch (err) {
    toast('Could not connect to server. Check that XAMPP is running and the database is set up.', 'error');
    console.error('Enroll error:', err);
  } finally {
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-plus"></i> Enroll'; }
  }
}

async function toggleSessionAccess(uid, grant) {
  try {
    await api('interns.php',{method:'PUT',body:JSON.stringify({action:'toggle_session_access',user_id:uid,session_access:grant?1:0})});
    toast(grant?'Session access granted.':'Access revoked.', grant?'success':'info');
    loadAdminInterns();
  } catch { toast(grant?'Granted. (Demo)':'Revoked. (Demo)', 'success'); }
}

async function setInternStatus(uid, status) {
  try {
    await api('interns.php',{method:'PUT',body:JSON.stringify({action:'set_status',user_id:uid,status})});
    toast(`Intern set to ${status}.`,'success'); loadAdminInterns();
  } catch { toast(`Status updated. (Demo)`,'success'); }
}
...
async function deleteIntern(uid) {
  ...
  try {
    ...
    toast('Intern removed.','success'); loadAdminInterns();
  } catch { toast('Removed. (Demo)','success'); }
}
...
function populateInternDropdown(elId) {
  ...
  el.innerHTML = '<option value="">All Interns (Broadcast)</option>' +
    allInterns.map(i=>`<option value="${i.user_id}" ${curr==i.user_id?'selected':''}>${i.full_name||i.username} (${i.intern_id||'—'})</option>`).join('');
}
...
async function loadNotifications() {
  try {
    ...
    // show dot on both admin and intern nav
    ...
    if (adminEl) {
      if (!n.length) { adminEl.innerHTML='<div class="empty"><i class="fas fa-bell"></i><p>No notifications.</p></div>'; }
      else adminEl.innerHTML = _renderNotifList(n);
    }
    // Intern notifications page
    const internEl=document.getElementById('intern-notif-list');
    if (internEl) {
      if (!n.length) { internEl.innerHTML='<div class="empty"><i class="fas fa-bell"></i><p>No notifications.</p></div>'; }
      else internEl.innerHTML = _renderNotifList(n);
    }
    // Dashboard doc alert for interns
    const docNotifs = n.filter(x=>x.type==='document_request'&&!x.is_read);
    const dashAlert = document.getElementById('dash-doc-alert');
    const dashAlertText = document.getElementById('dash-doc-alert-text');
    if (dashAlert && docNotifs.length) {
      dashAlert.style.display='flex';
      if(dashAlertText) dashAlertText.textContent = docNotifs[0].message + (docNotifs.length>1?` (+${docNotifs.length-1} more)`:'');
    }
  } catch {}
}

function _renderNotifList(n) {
  return n.map(x=>`<div class="notif-item">
...
async function markAllRead() {
  try { await api('notifications.php',{method:'PUT',body:JSON.stringify({})}); loadNotifications(); }
  catch { toast('Marked all read. (Demo)','success'); }
}
...
async function api(endpoint, opts={}) {
  ...
  const defs = {
    ...
    credentials: 'include'   // 'include' ensures session cookies are sent cross-path
  };
...
function toast(msg, type='') {
  const c = document.getElementById('toast-box'); if(!c) return;
...
function docLabel(t) {
  return {resume:'📄 Resume',endorsement:'📜 Endorsement',application:'📝 Application',
    nda:'🔒 NDA',waiver:'⚠️ Waiver',medical:'🏥 Medical',other:'📁 Other'}[t]||t;
}
